...
RUN apt-get update && apt-get install -y --no-install-recommends \
        ...
        libtiff6 fontconfig python3 python3-pip ca-certificates curl \
        poppler-utils \
    && rm -rf /var/lib/apt/lists/*
...
# --- 字体安装（apt 开源字体 + fonts.tar.xz + Noto Sans SC 生成 + 缺失字体名别名）---
# 所有字体逻辑都在 fonts_setup.sh 一个脚本里维护：
#   apt 装思源黑体 CJK/文泉驿/Carlito·Caladea；下载解压 fonts.tar.xz（宋体/黑体/楷体/仿宋/方正）；
#   fonttools 从 NotoSansCJK.ttc 抽 SC 字面生成独立 NotoSansSC（避免 ttc 第 0 面问题）；
#   写入 scan 别名：微软雅黑/Microsoft YaHei/Microsoft YaHei UI/Noto Sans/等线/DengXian → 思源黑体，
#   Calibri/Cambria → Carlito/Caladea（度量兼容 OFL 替代）。
# 注：微软雅黑为微软专有字体，不随镜像再分发（版权），故用同设计的思源黑体映射。
