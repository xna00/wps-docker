...
## v1.6.1 综合压测 + WPS daemon 长期运行退化观察（2026-08-17）

可读性重构后综合压测（12 路 docx / 小文件 1-24 并发 / big / mega / 混合 30 并发 ×2）：
- **异常**：连续多场景压测后转换普遍慢一倍（big 1.1s→2.2s、mega 7s→14s、小文件并发24 QPS 17→10，
  mega 并发4 新版 28.97s ≈ 旧版串行 28.80s，并行优势消失）。
- **重启容器后完全恢复**：mega 单发 7.78s、mega 并发4 14.71s（≈MAX=3 预期）、big 并发8 QPS 1.87、
  小文件并发24 QPS 16.62——与历史基准一致，**重构零性能回归**。

**运维观察**：WPS daemon 长期运行（连续大量转换）会退化导致转换变慢约一倍，重启容器即恢复。
与早期发现同源（残留实例多时 getApplication 直接 RPC_E_TIMEOUT）。生产建议：转换吞吐异常下降时
优先重启容器，无需改代码。
