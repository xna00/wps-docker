...
## v1.6.1 冷启动错峰间隔实测收缩（2026-08-17）

讨论点：「锁已保证冷启动串行化，为何还要 sleep 2s？」——实验一验证的是「串行+2s」组合，
从未单独验证间隔必要性。实测收缩（12 路并行 docx，各档 1 轮）：

| COLDSTART_STAGGER | 墙钟 | 成功率 | 异常 |
|---|---|---|---|
| 2.0s（原默认） | 27.2s | 12/12 | 0 |
| 1.0s | 14.5s | 12/12 | 0 |
| 0.5s（新默认） | 9.6s | 12/12 | 0 |
| 0.2s | 5.2s | 12/12 | 0 |

结论：串行化由文件锁保证，间隔只需给 daemon 留调度缓冲；2s 过度保守。
默认值 2.0s → 0.5s（2.5 倍余量），激进可调 0.2s（COLDSTART_STAGGER 环境变量）。

## v1.6.1 冷启动间隔彻底去除（零间隔实验，2026-08-17）

讨论质疑：「getApplication 返回 ≠ daemon 安定？那时间估算不也不安全？」——两层结论：

1. **"daemon 收尾窗口"推测被证伪**：`getApplication` 是**同步 RPC**，返回即 daemon
   处理完成的时刻，实例未就绪时该调用不会成功返回（E_FAIL 或阻塞）。所以"返回"本身就是
   就绪信号，无需再等"安定"。
2. **时间估算的隐患用"消灭时间"解决**：并发冷启动失败的根因是并发竞态，文件锁串行化后
   每个 getApplication 同步完成、下一个才开始，天然安全——2s 间隔是实验一组合里的冗余。

实测（COLDSTART_STAGGER=0，纯锁串行，3 轮 12 路并行 docx）：
...
对比：2.0s→27.2s / 0.5s→9.6s / 0.2s→5.2s / **0→3.0s**。
默认值改为 0（参数保留供极端环境微调）。

## v1.6.1 MAX_WORKERS 全局容量护栏（2026-08-17）

背景：弹性模型并发 N 就创建 N 个 worker（0.4GB/实例），突发峰值无内存上限。
方案（用户拍板）：全局上限 + 排队带超时 + 默认 3。

- `MAX_WORKERS`（默认 3，0=不限制）：asyncio.Semaphore 控制「池中存活 worker 数 ≤ MAX」
  （含 idle 备用），额度在 worker 销毁/死亡/超时移除时归还，idle 复用不新增。
- 无 idle 时 acquire 排队等额度，`QUEUE_TIMEOUT`（默认 60s）超时返回 503。
- health 新增 total/max_workers。

实测：
- 并发 8 小文件：2.29s 8/8 成功（排队消化）；大文件 8 并发 29.98s（3 并行，容量受限）。
- 压测后 total=1（收敛），容器 WPS 实例 ≤3，内存有界。
- 503 路径：3 个慢请求占满 + 第 4 个（QUEUE_TIMEOUT=5）→ 排队 5.0s → 503。

## v1.6.1 MAX_WORKERS 取值压测（2026-08-17）

对 MAX_WORKERS=1/3/8/16 四档压测（并发 12，小文件 0.09s + 大文件 1.1s，峰值实例后台采样）：

| MAX | 小文件墙钟/QPS | 大文件墙钟/QPS | 峰值实例 | 峰值内存 |
|---|---|---|---|---|
| 1 | 1.15s / 10.4 | 14.04s / 0.85 | 2 | ~0.8GB |
| **3** | **0.99s / 12.1** | **6.30s / 1.90** | 4 | ~1.6GB |
| 8 | 2.42s / 5.0 | 6.76s / 1.78 | 9 | ~3.6GB |
| 16 | 3.47s / 3.5 | 7.00s / 1.71 | 9 | ~3.6GB |

结论：
- **MAX=3 是双场景甜点**（小文件/大文件并发 12 均最优），默认 3 维持。
- **MAX 不是越大越好**：冷启动串行化是根本瓶颈——并发 N 全量并行冷启动
  （0.3s×N 串行）的开销抵消并行收益；小文件场景 MAX 越大越慢（16→3.47s）。
- 内存峰值 ≈ MAX+预热实例（MAX×0.4GB）。

## v1.6.1 巨大文件压测 + MAX 取值修正（2026-08-17）

giga.docx（80000 段，单请求 ~26s）并发 4：

| 版本 | 墙钟 | QPS | 提速 |
|---|---|---|---|
| 旧版 v1.6.0（单例串行） | 101.8s | 0.04 | — |
| 新版 MAX=3 | 50.3s | 0.08 | 2.0× |
| 新版 MAX=8 | 29.9s | 0.13 | **3.4×** |

**MAX 最优值随文件大小变化**（修正"MAX=3 一概最优"）：
- 转换 ≲ 冷启动（小文件）：冷启动串行化是瓶颈 → MAX=3 最优；
- 转换 ≫ 冷启动（大文件 7s/26s）：并行收益主导 → **MAX 越大越好**（26s 文件并发 4：
  MAX=8 的 29.9s vs MAX=3 的 50.3s）。
- 结论：MAX_WORKERS 按业务文件大小调——小文件业务 3，大文件业务 8~16。

## v1.6.1 挂死格式实测与入口拒绝（2026-08-17）

实测各格式转换表现：

| 格式 | 结果 |
|---|---|
| odp（wpp ODF） | 504 挂死（31s 超时） |
| **ods（et ODF）** | **挂死**（65s+ 无响应，worker 占满 TASK_TIMEOUT） |
| odt（wps ODF） | 正常 200 |
| doc（docx 内容改名） | 正常 200（WPS 智能识别） |
| ppt / xls（docx 内容改名，跨组件） | **挂死**（类型不匹配，wpp/et Open 错类型永久阻塞） |
| txt / html | 正常 |
...
## v1.6.1 真实文档测试 + idle 额度锁死 bug 修复（2026-08-17）

### 真实文档测试（sample.cat 下载）
| 文件 | 规模 | 单发 | PDF 输出 |
|---|---|---|---|
| sc1.docx | 17 页估算 | 0.77s | — |
| **sc4.docx** | **60.7 万字符 / 13.8MB / 估算 1200 页** | **5.2s** | **175 页 / 6.2MB** |
| sc1.xlsx | 391 行 | 0.93s | 7 页 |
| sc2.pptx | 2 页 | 0.75s | 2 页 |

### 发现并修复：idle 额度占满锁死动态创建（假串行）
**现象**：sc4.docx 并发 3 耗时 5.0/10.0/15.4s（严格等差 = 纯串行），新版与旧版同速。
现场解剖：2s 时 health 显示 wps busy=1，容器内只有 1 个 wps 实例——第 2、3 个请求**没新建 worker**。

**根因**：`MAX_WORKERS=3` 时，三类型各留 1 个 idle = 3 = MAX → 额度信号量计数 0（locked）。
第 2 个同类型请求 acquire：无 idle → `_sem.locked()` 为 True → 跳过新建 → `_cond.wait()` 等唤醒。
**idle 备用占满额度，动态创建被锁死** → 同类型并发只能等第一个完成（假串行）。
（此前压测容器通常只有 wps idle、额度有空余，故未暴露；真实文档测试把三类型都拉起后触发。）

**修复**（acquire）：无 idle 且额度被占满时，牺牲一个**他类型** idle（备用缓存，重建 ~0.3s）
腾出额度再新建本类型 worker。额度语义「存活 ≤ MAX」不变，内存上限不变。

**验证**：
- sc4 并发 3：15.2s 串行 → **6.4s 真并行**（5.4/5.8/6.4s，≈单发+冷启动）；
- 回归：docx 并发 6 后 xlsx/pptx 正常重建、小文件并发 8 QPS 7.45、12 路 docx 1.2s/12、混合 30 并发 30/30。
- **已知代价**：3 类型同时爆发（混合 30 并发）有「杀-建乒乓」，墙钟 1.2s→7.2s，功能正常无 503；
  单类型为主的真实业务不受影响。

## v1.6.1 图片密集文档测试（2026-08-17）

真实图片（提取自 sc4.docx 的 47 张，13.8MB）构造图片密集文档，验证 WPS 图片渲染：

| 文档 | 图片数 | 单发 | PDF 输出 | 渲染验证 |
|---|---|---|---|---|
| real_img.pptx（40 页每页 1 图） | 40 | 2.09s | 40 页 / 4.1MB | **pdfimages 40/40 张** |
| real_img.docx（47 图图文混排） | 47 | 2.74s | 39 页 / 4.7MB | **pdfimages 47/47 张** |

- 并发 2（49.9MB 大文件）：2.8s / 3.7s 并行正常。
- 转换期间 WPS RSS 峰值 **811MB**（图片渲染较纯文本 ~400MB 高约 1 倍，稳定无爆炸）。
- 压测后无泄漏（实例/僵尸/health 正常）。
- 结论：WPS 图片渲染 100% 成功、性能良好；图片密集是内存敏感场景但峰值可控
  （单实例 ~0.8GB，MAX_WORKERS=3 时图片密集高峰 ~2.4GB 上限内）。
